...
class MoonlinkQueue {
    ...
    add(data, position) {
        ...
        let queue = this.db.get(`${this.botId}.queue.${this.guildId}`);
        ...
        if (typeof position === 'undefined' || position > queue.length + 1) {
            if (Array.isArray(queue)) {
                this.db.push(`${this.botId}.queue.${this.guildId}`, data);
            }
            else if (queue && queue.length > 0 && queue[0]) {
                queue = [queue, data];
                this.db.set(`${this.botId}.queue.${this.guildId}`, queue);
            }
            else {
                this.db.push(`${this.botId}.queue.${this.guildId}`, data);
            }
    ...
    first() {
        let queue = this.db.get(`${this.botId}.queue.${this.guildId}`) || null;
        if (!queue)
            return null;
        if (Array.isArray(queue)) {
            return queue[0];
    ...
    clear() {
        ...
        if (!queue)
            return false;
        this.db.delete(`${this.botId}.queue.${this.guildId}`);
        return true;
    }
    get size() {
        return this.db.get(`${this.botId}.queue.${this.guildId}`)
            ? this.db.get(`${this.botId}.queue.${this.guildId}`).length
            : 0;
    }
    remove(position) {
    ...
    get all() {
        return this.db.get(`${this.botId}.queue.${this.guildId}`)
            ? this.db.get(`${this.botId}.queue.${this.guildId}`)
            : null;
    }
}
